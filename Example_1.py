            ...
            for j in range(first_row, sheet.max_row):
                if(sheet.cell(row=j, column=munch_column).value != None):
                    ...
                    if field not in field_value_sum:
                        field_value_sum[field] = int(sheet.cell(
                            row=j, column=munch_column+1).value)
                    else:
                        field_value_sum[field] += int(sheet.cell(
                            row=j, column=munch_column+1).value)

    field_value_sum_sorted = sorted(
        field_value_sum.items(), key=lambda x: x[1], reverse=True)
...
if __name__ == '__main__':
    ...
    try:
        ...
        for i in range(1, format_sheet.max_column):
            if format_sheet.cell(row=4, column=i).value == "Hits":
                ...
                column_name_to_index[format_sheet.cell(
                    row=4, column=i-1).value] = i-1
    except:
    ...
    try:
        if column_names[column_to_sum] == "All":
            ...
            sum_sorted_fields[column_names[column_to_sum]] = \
                munch(
                    wb, key, column_name_to_index[column_names[column_to_sum]])
    except:
        print(".xlsx file formating error, check all hit field only have int")
        quit()

    column_total = {}
    for i in sum_sorted_fields:
        total_hits = 0
        for j in range(0, len(sum_sorted_fields[i])):
            total_hits += sum_sorted_fields[i][j][1]
        column_total[i] = total_hits
    ...
    if key[0] and key[1]:
        ...
        keyPrint = str(key[1])
    print("--------------------")
    print("%s traffic looks like:" % keyPrint)

    for i in sum_sorted_fields:
        if sum_sorted_fields[i]:
            print("--------------------")
        for j in range(0, len(sum_sorted_fields[i])):
            hit_percentage = (
                float(sum_sorted_fields[i][j][1]) / float(column_total[i]))*100.0
            print("%s %s had %s hits %s%% of traffic"
                  % (i,
                     str(sum_sorted_fields[i][j][0]),
                     str(sum_sorted_fields[i][j][1]),
                     str(round(hit_percentage, 2))))

    # print("--------------------")
    # input_key = input("[1] Save to Output#.Xlsx (Eny other key to exit) ")
    # if inputKey == "1":
    #     n = 1
    ...
    #             for j in range(0, len(sum_sorted_fields[i])):
    #                 hitPercentage = (
    #                     float(sum_sorted_fields[i][j][1]) / float(fieldTotalHits[i]))*100.0
    #                 worksheet.write(j+2, c+0, sum_sorted_fields[i][j][0])
    #                 worksheet.write(j+2, c+1, sum_sorted_fields[i][j][1])
